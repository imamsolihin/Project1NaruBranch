            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700 p-8">
                <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" class="space-y-6">
                    ...
                    @foreach($divisions as $division)
                        <option value="{{ $division->id }}" {{ old('division_id', $project->division_id) == $division->id ? 'selected' : '' }}>
                            {{ $division->name }}
                        </option>
                    @endforeach
                        </select>
                    ...
                    @foreach($users as $u)
                        <option value="{{ $u->id }}" {{ old('assigned_user_id', $project->assigned_user_id) == $u->id ? 'selected' : '' }}>
