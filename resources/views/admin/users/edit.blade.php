                    ...
                    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="role" class="w-full rounded-xl border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white" required>
                                ...
                                @if(auth()->user()->role === 'super_admin' || auth()->user()->role === 'admin')
                                    <option value="wakil_admin" {{ old('role', $user->role) == 'wakil_admin' ? 'selected' : '' }}>Wakil Admin</option>
                        ...
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-6" x-data="{ showPassword: false }">
                            ...
                            <div>
                                ...
                                <input :type="showPassword ? 'text' : 'password'" name="password" autocomplete="new-password" class="w-full rounded-xl border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white" placeholder="Biarkan kosong jika tidak diganti">
                                @error('password') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>

                            <div class="mt-4">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Konfirmasi Password Baru</label>
                                <input :type="showPassword ? 'text' : 'password'" name="password_confirmation" autocomplete="new-password" class="w-full rounded-xl border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
                            </div>
                        </div>
